...
def evaluate(loader, model):
    ...
    for batch in tqdm(loader, desc="Evaluating"):
        ...
        with torch.no_grad():
            tmp_loss, logits = model(inp_ids, inp_masks, labels=labels)
...
def train(loader, model_dir, lr=2e-5, num_labels=18, epochs=4, save_steps=3000, test_loader=None, eval_callback=None):
    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(tqdm(loader, desc='Training')):
            ...
            loss, logits = model(inp_ids, inp_masks, labels=labels)
            model.zero_grad()
            loss.backward()
            optimizer.step()
            ...
            # tensorboard
            if i % 100 == 0:
                writer.add_scalar(f'Train/Loss-Epoch{epoch}', loss.mean().item(), i)
                writer.add_scalar(f'Train/Accuracy-Epoch{epoch}', precision_score(ys_true, ys_pred, average='weighted'), i)
                writer.flush()
        ...
        writer.add_scalars('TrainTest/Loss', {'train': train_loss, 'test': test_loss}, epoch)
        writer.add_scalars('TrainTest/Accuracy', {'train': train_acc, 'test': test_acc}, epoch)
        writer.flush()
...
def main():
    ...
    MODEL_DIR = 'distilbert-base-multilingual-cased'
    MAX_LEN = 512
    BATCH_SIZE = 8
    EPOCHS = 4
    ...
    train_dataset = convert_examples_to_features(examples, max_length=MAX_LEN, tokenizer=DistilBertTokenizer.from_pretrained(MODEL_DIR))
    train_loader = DataLoader(train_dataset, shuffle=True, batch_size=BATCH_SIZE)

    # test dataloader
    examples = DataProcessor().get_test_examples(content[NUM_TRAIN_DATA:], target[NUM_TRAIN_DATA:])
    test_dataset = convert_examples_to_features(examples, max_length=MAX_LEN, tokenizer=DistilBertTokenizer.from_pretrained(MODEL_DIR))
