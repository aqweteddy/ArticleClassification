    ...
    for epoch in range(epochs):
        ...
        for i, batch in enumerate(tqdm(loader, desc='Training')):
            ...
            if (global_step+1) % 50 == 0:
                writer.add_scalar(f'Train/Loss-Epoch', loss.mean().item(), global_step=global_step+1)
            ...
            if (global_step+1) % 500 == 0:
                ...
                writer.add_text("history", f"step: {global_step}\ttrain_acc: {train_acc}\ttrain_loss: {train_loss}", global_step=global_step+1)
                writer.flush()
